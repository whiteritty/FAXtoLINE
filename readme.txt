...
Azureアプリを設定した前提のアプリでブラウザ認証なので何も収集しません

【重要】
※DropboxのAPI取得が4時間なので使えない、DropBoxのAPIの取得する必要はない
Onedrive版なので一度動作させ始めたら最大360日までAPIが持続するのでオススメです
LINEトークンと、Azureアプリトークンの取得が必要です、下記に記載
まだPython版なのでアプリ化まで安定動作確認中

【制限】
LINE通知は月に200件まで
そのレベルであればondriveのAPIに制限は来ない
ただ保存容量を使うので7日たったものを24時間に一回削除

【変更履歴】
1.1 リフレッシュトークン保持、有効時自動入力、アドレス帳に未登録番頭自動登録、西暦で始まる番号は登録しない
1.01 Onedrive版　正規化バグ修正
...
一致しない場合はアドレス帳に登録する
正規化後西暦で始まる場合（例：2025）はアドレス帳に登録しない
処理結果をログに記録。
